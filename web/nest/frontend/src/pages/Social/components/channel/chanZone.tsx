...
export default function ChanZone({handleSelect, login, currentChan}) {
    const [createForm, setCreateForm] = useState(false);
    const [joinForm, setJoinForm] = useState(false);
    const [chanList, setChanList] = useState([])

    useEffect(() => {
        const url = "http://localhost:3333/channel/mine?login=" + login;
        axios.get(url)
        .then((response) => {
    ...
    return (
        <div>
            ...
            <ChanList list={chanList} handleSelect={handleSelect}/>
        </div>
    )
}
