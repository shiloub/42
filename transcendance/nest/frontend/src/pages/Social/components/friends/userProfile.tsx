...
function Profile({userLogin, login, handleAdd, handleDel, handleBlock, handleUnblock, blockeds, friends}) {
    ...
    function handleClickAdd () {
        const url = "http://localhost:3333/users/addFriend";
        const data = {
            login: login,
            target: userLogin,
        }
    ...
    function handleClickDell () {
        const url = "http://localhost:3333/users/DelFriend";
        const data = {
            login: login,
            target: userLogin,
        }
    ...
    function handleClickBlock () {
        const url = "http://localhost:3333/users/blockUser";
        const data = {
            login: login,
            target: userLogin,
        }
    ...
    function handleClickUnblock () {
        const url = "http://localhost:3333/users/unblockUser";
        const data = {
            login: login,
            target: userLogin,
        }
...
export default function UserProfile({userLogin, setUser, handleAdd, handleUnblock, handleDel, handleBlock, login, blockeds, friends}) {
    ...
    function handleSearch() {
        ...
        const url = "http://localhost:3333/users/other?login=" + input;
        console.log(url);
        axios.get(url)
        .then((response) => {
