...
export default function SettingForm({role, handleLeave, handleDelete, channel}){
    ...
    useEffect (() => {
        const url_default = "http://localhost:3333/channel/default?name=" + channel.name;
        axios.get(url_default).then((reponse) =>{
        ...
        const url_all = "http://localhost:3333/channel/all?name=" + channel.name;
        axios.get(url_all).then((reponse) =>{
            if (reponse.data.users)
    ...
    function handlepswDelete() {
        const url = "http://localhost:3333/channel/setPassword";
        const data = {
            name: channel.name,
            password: ""
        }
    ...
    function handleSetAdmin() {
        if (selectedUserDefault !== ''){
            const url = "http://localhost:3333/channel/setAdmin";
            const data = {
    ...
    function handleKick(){
        const url = "http://localhost:3333/channel/kick";
        const data = {
            login: selectedUser,
            name: channel.name,
        }
    ...
    function handleBan(){
        const url = "http://localhost:3333/channel/ban";
        const data = {
            login: selectedUser,
            name: channel.name,
        }
    ...
    function handleInvite() {
        const url = "http://localhost:3333/channel/invite";
        const data = {
            login: invited,
            name: channel.name,
        }
    ...
    function handleMute(){
        const url = "http://localhost:3333/channel/mute";
        const data = {
            login: selectedUser,
            name: channel.name,
        }
